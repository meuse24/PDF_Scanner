...
| PDF splitten | Einzelaktion Split | PDF mit >= 2 Seiten | Splitten, Thumbnails erzeugen, Teile speichern | ungueltige Split-Punkte, IO-Fehler | vorerst `SplitPdfUseCase` |
| Seiten neu anordnen | Einzelaktion Reorder | PDF mit >= 2 Seiten | Reihenfolge anwenden, Thumbnail erneuern, Datei/DB aktualisieren | ungueltige Reihenfolge, IO-Fehler | vorerst `ReorderPagesUseCase` |
...
- `ImportScanWorkflow`
- konsistenter Delete-Workflow fuer Datei- und DB-Konsistenz
- gemeinsamer `PdfEditingWorkflow` fuer Merge/Split/Reorder, falls weitere
  Reihenfolgelogik zwischen diesen Features entsteht
